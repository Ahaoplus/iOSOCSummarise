...
static const CGFloat internalWidth = 37.5f;
...
-(void)configParameter:(DirectionStyle)directionStyle viewArr:(NSArray<UIView*>*)viewArr tabWidth:(NSInteger)tabWidth tabHeight:(NSInteger)tabHeight index:(NSInteger)index block:(TabClickBlock) clickBlock{
    ...
    if(_direction==horizontal){
        ...
        [_viewArr enumerateObjectsUsingBlock:^(UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            [obj sizeToFit];
            allWidth+= obj.frame.size.width;
            
        }];
        __block CGFloat x = (kScreenWidth - allWidth - internalWidth * (_viewArr.count -1))/2;  //在中间
        if (x<0) {
            x=0.f;
        }
        [_viewArr enumerateObjectsUsingBlock:^(UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            CGFloat width = obj.frame.size.width;
            obj.frame=CGRectMake(x, 0, width, _tabHeight);
            x+=width + internalWidth;
            [self addSubview:obj];
...
-(void)updateTag:(NSInteger)index{
    ...
    CGFloat x = _viewArr[index].frame.origin.x;
    CGFloat width = _viewArr[index].frame.size.width;
    self->_tagIndex=index;
    self->_tagLine.frame=CGRectMake(x, self->_tabHeight-tagLineheight-0.5, width, tagLineheight);
...
-(void)updateTagLine:(NSInteger)index{
    ...
    if(_direction==horizontal){
        ...
        //标记线切换动画
        [UIView animateWithDuration:0.1 animations:^{
            self->_tagLine.frame=CGRectMake(self->_tabWidth-tagLineheight-0.5, self->_tabHeight*index, tagLineheight, self->_tabHeight);
        } completion:^(BOOL finished) {
            self->_tagIndex=index;
...
-(void)initView{
    ...
    self.tagLine.backgroundColor= [UIColor redColor] ;  //默认标记线为红色
    if (@available(iOS 11.0, *)) {
        self.backgroundColor = [UIColor colorNamed:@"EYSystemPageBGColor"];
    } else {
